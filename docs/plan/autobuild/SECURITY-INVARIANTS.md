...
- **Authorized relaxation (2026-08-20, Josh): HiClaw Matrix raw-HTTP client.** Ardyn MAY make a minimal, raw-fetch Matrix client-server connection to the HiClaw homeserver only: send m.room.message (m.text only) and receive via /sync. Conditions: deny-by-default room/sender allowlists; per-user isolation preserved; any gated action still passes approval + kill + audit + redaction; token from env / gitignored config/secret/ only - never logged, never in URLs or errors. No E2EE: m.room.encrypted events are skipped, never decrypted. STILL BANNED (unchanged): matrix-js-sdk / @matrix-org/* dependencies (raw fetch only) and any Matrix E2EE / encryption-key handling.
