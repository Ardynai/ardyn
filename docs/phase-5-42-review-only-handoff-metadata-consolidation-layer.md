...
Phase 5.42 adds a deterministic review-only handoff metadata consolidation
layer over the Phase 5.41 metadata handoff checkpoint. The output is
non-authorizing consolidation metadata for later review only.

Machine-readable Phase 5.42 artifact path:

`tests/fixtures/host-policy/phase5-42/review-only-handoff-metadata-consolidation-layer.json`

Focused Phase 5.42 test path:
...
The Phase 5.42 helper consumes only Phase 5.41 review-only metadata handoff
checkpoint state and may produce only:

- review-only handoff metadata consolidation layer state
- digest-only source metadata handoff checkpoint references
- non-authorizing consolidation summary metadata
...
Phase 5.42 rejects missing, malformed, invalid timestamp, empty, conflicting,
...
top-level, unsafe nested handoff/checkpoint/consolidation metadata, and
execution-signal-looking metadata handoff checkpoint state.

Phase 5.42 also rejects unsafe nested source metadata, including nested approval
grant, approval decision, reviewer assignment, reviewer routing, evaluator
execution/result, runtime effect, command exposure, and missing or malformed
source digest state.

Rejected inputs produce no handoff metadata consolidation layer state.

## Handoff Metadata Consolidation Layer

The Phase 5.42 consolidation layer records:

- the source Phase 5.41 metadata handoff checkpoint schema, kind, review mode,
  timestamp, and digest
- digest-only upstream checkpoint handoff references
- installed cleanup/hardening toolkit validation evidence references
- a handoff metadata consolidation summary with reviewer routing, reviewer
...
The handoff metadata consolidation layer is not:
...
Phase 5.42 records these required validation commands:
