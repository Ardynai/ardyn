...
test("ardyn emit-session-events --dry-run writes deterministic session-event JSONL to stdout only", async () => {
  ...
  assert.equal(stderr, "");
  assert.equal(stdout, formatSessionEventsJsonl(expectedEvents));
...
test("ardyn emit-session-events rejects missing dry-run mode and bad local input paths on stderr only", async () => {
  assertCliFailure(
    await runCliFailure([
      "emit-session-events",
      "--manifest",
    ...
    /Only emit-session-events --dry-run is available in Phase 4\.0A\./
  );
...
test("Phase 4 CLI source does not add live stdin loops, listeners, subprocesses, or adapter calls", async () => {
  const source = await readFile("apps/cli/src/index.mjs", "utf8");
